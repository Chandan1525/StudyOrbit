...
import {
  Home, Search, MessageCircle, User, Bell, Heart, Users, 
  MessageSquare, Share2, Bookmark, Globe, Smartphone, Brain, 
  Code2, Sliders, MoreVertical, CheckCircle2, PlayCircle, Zap, 
  WifiOff, RefreshCw, AlertCircle, Send, X, Trash2
} from "lucide-react";
...
// ── Data ──────────────────────────────────────────────────────
const COMMUNITIES = [
  { id: 1, tag: "WEB", name: "Web Dev", members: "12.5K", icon: Globe, live: true, bg: "linear-gradient(135deg,#4f46e5,#7c3aed)" },
  { id: 2, tag: "APP", name: "App Dev", members: "8.3K", icon: Smartphone, live: false, bg: "linear-gradient(135deg,#0284c7,#3b82f6)" },
  { id: 3, tag: "ML", name: "ML Space", members: "15.7K", icon: Brain, live: true, bg: "linear-gradient(135deg,#059669,#10b981)" },
  { id: 4, tag: "DSA", name: "Algorithms", members: "9.1K", icon: Code2, live: false, bg: "linear-gradient(135deg,#db2777,#f43f5e)" },
];
...
const TOPICS = [
  ...
  { label: "DSA", icon: "💻", gradient: "linear-gradient(135deg,#db2777,#f43f5e)", endpoint: "DSA" },
];
  ...
  return (
    <div className="flex flex-col items-center justify-center py-14 px-6 text-center bg-white dark:bg-slate-900 rounded-[30px] border border-gray-100 dark:border-slate-800 shadow-sm">
      ...
      <h3 className="font-bold text-gray-800 dark:text-slate-100 mb-2">
        {message.includes("connect") ? "Can't reach server" : "Something went wrong"}
      </h3>
      <p className="text-sm text-gray-400 dark:text-slate-400 mb-5 leading-relaxed max-w-xs">{message}</p>
      <button onClick={onRetry} className="flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-accent text-white text-sm font-semibold shadow-lg transition-transform active:scale-95">
...
// ── Comment section ───────────────────────────────────────────
function CommentSection({ post, currentUser }: { post: any; currentUser: any }) {
  ...
  const submitComment = async () => {
    ...
    try {
      const res = await axios.post(`${API}/api/posts/${post._id}/comments`, { text: text.trim() }, { headers: authHeader() });
    ...
    <div className="mt-4 pt-4 border-t border-gray-50 dark:border-slate-800">
        ...
        <div className="space-y-3 mb-4 max-h-48 overflow-y-auto">
          {comments.map((c: any) => (
            <div key={c._id} className="flex gap-2.5">
              <img
                src={c.user?.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(c.user?.name || "U")}&background=6366f1&color=fff&size=32`}
                alt=""
                className="w-7 h-7 rounded-full object-cover flex-shrink-0 mt-0.5"
                onError={(e) => { (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=U&background=6366f1&color=fff&size=32`; }}
              />
              <div className="flex-1 bg-gray-50 dark:bg-slate-800/50 rounded-2xl px-3 py-2">
                <div className="flex items-center gap-2 mb-0.5">
                  <span className="text-xs font-bold text-gray-800 dark:text-slate-200">{c.user?.name || c.user?.username || "Student"}</span>
                  <span className="text-[10px] text-gray-400 dark:text-slate-500">{c.createdAt ? new Date(c.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short" }) : ""}</span>
                </div>
                <p className="text-xs text-gray-700 dark:text-slate-300 leading-relaxed">{c.text}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex gap-2.5 items-center">
        ...
        <div className="flex-1 flex items-center gap-2 bg-gray-50 dark:bg-slate-800/50 rounded-2xl px-3 py-2 border border-gray-100 dark:border-slate-700 focus-within:border-accent transition-colors">
          ...
          />
          <button onClick={submitComment} disabled={!text.trim() || isPosting} className="text-accent disabled:text-gray-300 dark:disabled:text-gray-600 transition-colors flex-shrink-0">
            <Send size={14} />
          </button>
        </div>
      </div>
...
// ── Post card ─────────────────────────────────────────────────
function PostCard({ post, currentUser, onDelete }: { post: any; currentUser: any; onDelete?: (id: string) => void }) {
  const userId = currentUser?._id || currentUser?.id || "";

  const [liked, setLiked] = useState(() => post.likes?.some((id: any) => id === userId || id?._id === userId) ?? false);
  const [likesCount, setLikesCount] = useState(() => post.likes?.length ?? 0);
  
  // 🔥 Sync initial save state with current user's saved list
  const [saved, setSaved] = useState(() => {
    return currentUser?.savedPosts?.includes(post._id) || false;
  });
  
  const [showComments, setShowComments] = useState(false);
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [isDeleting, setIsDeleting] = useState(false);
  const isAuthor = userId === post.author?._id || userId === post.author?.id;

  const getTagColors = (orbit: string) => {
    switch (orbit?.toUpperCase()) {
      case "WEB": return { color: "#059669", bg: "#d1fae5" };
      case "ML":
      case "AI": return { color: "#7c3aed", bg: "#f3e8ff" };
      case "APP": return { color: "#0284c7", bg: "#e0f2fe" };
      case "DSA": return { color: "#db2777", bg: "#fce7f3" };
      default: return { color: "var(--accent-color)", bg: "color-mix(in srgb, var(--accent-color) 15%, transparent)" };
    }
  };
  ...
  const handleDeletePost = async () => {
    ...
    setIsDeleting(true);
    try {
      await axios.delete(`${API}/api/posts/${post._id}`, { headers: authHeader() });
      setIsMenuOpen(false);
  ...
  const handleLike = async () => {
    ...
    try {
      const res = await axios.put(`${API}/api/posts/like/${post._id}`, {}, { headers: authHeader() });
      if (res.data && res.data.likes) {
        setLikesCount(res.data.likes.length);
        const userHasLiked = res.data.likes.some((id: any) => id === userId || id?._id === userId);
  ...
  const handleSave = async () => {
    if (!userId) return alert("Please login to save posts");

    const wasSaved = saved;
    setSaved(!wasSaved); // Optimistic UI update

    try {
      const res = await axios.put(
        `${API}/api/posts/save/${post._id}`,
        {},
        { headers: authHeader() }
      );

      // Update local storage so the save persists and Profile page can read it
      if (currentUser) {
      ...
      setSaved(wasSaved); // Revert UI if DB fails
  ...
  const handleDashboardShare = () => {
    if (typeof window !== "undefined") {
      ...
      navigator.clipboard.writeText(postLink).then(() => alert("🔗 Link copied to clipboard!")).catch((err) => console.error("Failed to copy link: ", err));
    }
  };
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-[30px] p-5 border border-gray-100 dark:border-slate-800 shadow-sm transition-colors">
      <div className="flex items-start justify-between">
        <Link href={`/profile/${post.author?._id}`} className="flex items-center gap-3 group cursor-pointer">
          <img
            ...
            onError={(e) => { (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=U&background=6366f1&color=fff`; }}
          />
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-xs text-gray-400 dark:text-slate-500">@{post.author?.username || "user"}</span>
        ...
        <div className="relative">
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="text-gray-400 hover:text-gray-600 dark:hover:text-slate-300 p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition">
      ...
      <div className="mt-4">
        {post.orbit && (
          <span className="inline-flex px-3 py-1 rounded-full text-xs font-bold mb-3" style={{ color: colors.color, background: colors.bg }}>
            {post.orbit}
          </span>
        )}
        ...
        {post.hashtags?.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-3">
            {post.hashtags.map((tag: string, idx: number) => (
              <span key={idx} className="text-accent text-xs font-semibold">#{tag}</span>
            ))}
          </div>
        )}
      </div>
      ...
      {post.image && (
        <div className="mt-4 rounded-2xl overflow-hidden bg-gray-50 dark:bg-slate-800/50 border border-gray-100 dark:border-slate-800">
          <img src={post.image} alt="post" className="w-full h-auto max-h-[500px] object-contain" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
        </div>
      )}

      <div className="flex items-center justify-between mt-5 pt-4 border-t border-gray-50 dark:border-slate-800">
        <div className="flex items-center gap-5">
          <button onClick={handleLike} className={`flex items-center gap-1.5 transition-all ${liked ? "text-rose-500 scale-110" : "text-gray-400 dark:text-slate-500 hover:text-rose-400 dark:hover:text-rose-400"}`}>
            <Heart size={19} fill={liked ? "#f43f5e" : "none"} strokeWidth={liked ? 0 : 2} />
            <span className="text-sm font-semibold">{likesCount}</span>
          </button>
          <button onClick={() => setShowComments((v) => !v)} className={`flex items-center gap-1.5 transition-colors ${showComments ? "text-accent" : "text-gray-400 dark:text-slate-500 hover:text-accent"}`}>
            <MessageSquare size={19} />
            <span className="text-sm font-semibold">{post.comments?.length ?? 0}</span>
          </button>
          <button onClick={handleDashboardShare} className="flex items-center gap-1.5 text-gray-400 dark:text-slate-500 hover:text-green-500 transition-colors active:scale-95 cursor-pointer">
            <Share2 size={19} />
            <span className="text-sm font-semibold">{post.shares ?? 0}</span>
          </button>
        </div>
        
        {/* 🔥 Hooked up the true handleSave function here */}
        <button onClick={handleSave} className={`transition-all active:scale-90 ${saved ? "text-accent" : "text-gray-400 dark:text-slate-500 hover:text-accent"}`}>
          <Bookmark size={19} fill={saved ? "var(--accent-color)" : "none"} className={saved ? "text-accent" : ""} />
        </button>
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      try {
        const res = await axios.get(url, { headers: authHeader(), timeout: 30000 });
        let data: any[] = [];
        ...
        if (err.code === "ECONNREFUSED" || err.message?.includes("Network Error")) {
          setFetchError("Cannot connect to server. Make sure your backend is running on port 5000.");
        } else if (err.response?.status === 401) {
          setFetchError("Session expired. Please login again.");
          setTimeout(() => router.push("/auth/login"), 2000);
        } else if (err.response?.status === 404) {
          setFetchError(`API route not found: ${url} — check your postRoutes.js`);
        } else {
          setFetchError(err.response?.data?.message || err.message || "Failed to load posts.");
        }
      } finally {
        setIsLoading(false);
      }
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300 pb-24">
      ...
      <div className="sticky top-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-gray-100 dark:border-slate-800 px-5 py-4 flex items-center justify-between transition-colors">
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            <button onClick={() => router.push("/notifications")} className="relative p-2 bg-gray-100 dark:bg-slate-800 rounded-full hover:bg-gray-200 dark:hover:bg-slate-700 transition">
      ...
      <div className="px-5 pt-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={() => router.push("/community")} className="text-accent font-bold text-sm hover:opacity-80 active:scale-95 transition-all">
            View all
          </button>
        </div>
        <div className="flex gap-4 overflow-x-auto pb-2" style={{ scrollbarWidth: "none" }}>
          {COMMUNITIES.map((c) => (
            <div key={c.id} onClick={() => router.push(`/community?channel=${c.tag}`)} className="flex-shrink-0 w-[150px] h-[180px] rounded-[32px] p-5 relative overflow-hidden cursor-pointer active:scale-[0.97] transition-transform shadow-sm" style={{ background: c.bg }}>
              ...
              <div className="relative z-10 h-full flex flex-col justify-between">
                ...
                <div>
                  <h3 className="text-white text-[32px] font-black leading-none">{c.tag}</h3>
                  <p className="text-white/90 mt-2 font-semibold text-sm">{c.name}</p>
     ...
     {/* Search & Filter */}
      <div className="px-5 mt-6 relative z-30">
        <div className="flex items-center gap-3 relative">
          <div className="flex-1 bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800 flex items-center gap-3 px-4 py-3.5 shadow-sm transition-colors focus-within:ring-2 ring-accent">
            ...
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery("")} className="text-gray-400 hover:text-gray-600 dark:hover:text-slate-300 text-lg leading-none">×</button>
            )}
          </div>
          
          <button 
            onClick={() => setIsFilterOpen(!isFilterOpen)}
            className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg transition-all ${isFilterOpen ? "bg-accent/90 ring-2 ring-accent/50 text-white" : "bg-accent text-white"}`}
          >
            <Sliders size={18} />
          </button>
        </div>
        ...
        {isFilterOpen && (
          <>
            ...
            <div className="absolute right-5 top-[70px] w-48 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-2xl shadow-xl z-50 overflow-hidden py-1.5 animate-in fade-in zoom-in duration-200">
              <div className="px-4 py-2 border-b border-gray-100 dark:border-slate-800 mb-1">
                <span className="text-[10px] font-black uppercase tracking-wider text-gray-500 dark:text-gray-400">Sort By</span>
              </div>
              <button onClick={() => { setSortBy("newest"); setIsFilterOpen(false); }} className={`w-full text-left px-4 py-2.5 text-[13px] font-bold transition-colors flex items-center justify-between ${sortBy === "newest" ? "text-accent bg-accent/5" : "text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-slate-800/80"}`}>
                Latest First
                {sortBy === "newest" && <CheckCircle2 size={14} className="text-accent" />}
              </button>
              <button onClick={() => { setSortBy("popular"); setIsFilterOpen(false); }} className={`w-full text-left px-4 py-2.5 text-[13px] font-bold transition-colors flex items-center justify-between ${sortBy === "popular" ? "text-accent bg-accent/5" : "text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-slate-800/80"}`}>
                Most Liked
                {sortBy === "popular" && <CheckCircle2 size={14} className="text-accent" />}
              </button>
              <button onClick={() => { setSortBy("oldest"); setIsFilterOpen(false); }} className={`w-full text-left px-4 py-2.5 text-[13px] font-bold transition-colors flex items-center justify-between ${sortBy === "oldest" ? "text-accent bg-accent/5" : "text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-slate-800/80"}`}>
                Oldest First
                {sortBy === "oldest" && <CheckCircle2 size={14} className="text-accent" />}
              </button>
            </div>
          </>
        )}
      ...
      <div className="px-5 mt-8">
        <h2 className="text-xl font-black text-gray-900 dark:text-white mb-5 transition-colors">Discover Topics</h2>
        <div className="flex gap-4 overflow-x-auto pb-3" style={{ scrollbarWidth: "none" }}>
          {TOPICS.map((item) => {
            const active = activeInterest === item.label;
            return (
              <button key={item.label} onClick={() => setActiveInterest(item.label)} className="flex-shrink-0 w-[145px] h-[165px] rounded-[30px] p-5 relative overflow-hidden text-left active:scale-95 transition-transform" style={{ background: item.gradient }}>
                <div className="absolute -top-5 -right-5 w-28 h-28 bg-white/10 rounded-full" />
                <div className="relative z-10 h-full flex flex-col justify-between">
      ...
      <div className="px-5 mt-8 space-y-5">
        ...
        {isLoading ? (
          ...
          <ErrorState message={fetchError} onRetry={() => setRetryCount((c) => c + 1)} />
        ) : posts.length > 0 ? (
          [...posts]
            .sort((a, b) => {
          ...
          <div className="flex flex-col items-center justify-center py-14 text-center bg-white dark:bg-slate-900 rounded-[30px] border border-gray-100 dark:border-slate-800 shadow-sm transition-colors">
            ...
            <p className="text-sm text-gray-400 dark:text-slate-500">
              {searchQuery ? `No results for "${searchQuery}"` : "Be the first to post in this topic!"}
            </p>
          </div>
        )}
      </div>
