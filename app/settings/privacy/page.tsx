...
export default function PrivacySecurityPage() {
  ...
  const fetchCurrentSettings = async () => {
    try {
      ...
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:5000"}/api/users/profile/${currentUser.id || currentUser._id}`,
      );
  ...
  const handleToggle = async (type: "public" | "online") => {
    ...
    const newOnlineState =
      type === "online" ? !showOnlineStatus : showOnlineStatus;
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(
        `${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:5000"}/api/users/account`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <div className="max-w-md mx-auto px-5 pt-8 pb-24 space-y-6">
        {/* ── VISIBILITY SECTION ── */}
        <div>
          <h2 className="text-xs font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest mb-3 ml-1">
            Visibility
          </h2>
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-2 shadow-sm border border-gray-200/60 dark:border-slate-800">
            ...
            <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-slate-800">
              <div className="flex items-center gap-4">
                <div
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${isPublic ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-500" : "bg-gray-100 dark:bg-slate-800 text-gray-500"}`}
                >
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5 max-w-[200px] leading-tight">
                    {isPublic
                      ...
                      : "Only your connections can find you"}
                  </p>
                </div>
              </div>
              ...
              >
                <div
                  className={`w-5 h-5 rounded-full bg-white shadow transition-all duration-300 ${isPublic ? "translate-x-7" : "translate-x-0"}`}
                />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between p-4">
              <div className="flex items-center gap-4">
                <div
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${showOnlineStatus ? "bg-blue-50 dark:bg-blue-500/10 text-blue-500" : "bg-gray-100 dark:bg-slate-800 text-gray-500"}`}
                >
                  <Activity size={20} />
                </div>
                <div>
              ...
              >
                <div
                  className={`w-5 h-5 rounded-full bg-white shadow transition-all duration-300 ${showOnlineStatus ? "translate-x-7" : "translate-x-0"}`}
                />
              </button>
            </div>
          </div>
        ...
        {/* ── DANGER ZONE ── */}
        <div className="pt-4">
          <h2 className="text-xs font-black text-rose-500 uppercase tracking-widest mb-3 ml-1">
            Danger Zone
          </h2>
            ...
            <div className="flex items-start gap-4 mb-5">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-slate-400 leading-relaxed">
                  ...
                  projects, posts, and network data will be permanently erased.
                </p>
              </div>
            </div>
