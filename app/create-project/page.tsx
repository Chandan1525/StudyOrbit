...
import { ArrowLeft, FolderGit2, ExternalLink, Type, AlignLeft, Send } from "lucide-react";
...
export default function CreateProjectPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name || !formData.desc) return alert("Name and description are required!");
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-24 font-sans text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      <div className="sticky top-0 z-50 backdrop-blur-xl bg-white/80 dark:bg-slate-900/80 px-5 py-4 flex items-center justify-between border-b border-gray-100 dark:border-slate-800 transition-colors">
        ...
        <button 
          ...
          disabled={loading}
          className="flex items-center gap-2 px-4 py-2 bg-accent text-white font-bold text-sm rounded-full hover:opacity-90 transition disabled:opacity-50"
        >
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 mt-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white dark:bg-slate-900 p-5 rounded-[24px] border border-gray-100 dark:border-slate-800 shadow-sm transition-colors">
              <label className="flex items-center gap-2 text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">
                <GithubIcon size={16} className="text-gray-900 dark:text-white" /> GitHub URL
              </label>
              <input
                type="url"
                name="github"
            ...
            <div className="bg-white dark:bg-slate-900 p-5 rounded-[24px] border border-gray-100 dark:border-slate-800 shadow-sm transition-colors">
              <label className="flex items-center gap-2 text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">
                <ExternalLink size={16} className="text-blue-500" /> Live Demo URL
              </label>
              <input
                type="url"
                name="live"
